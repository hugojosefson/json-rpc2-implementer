...
describe("JsonRpc2Implementer", () => {
  ...
  describe("#call()", () => {
    ...
    it("should call sender and throw error", async function () {
      ...
      rpc.sender = (msg) => {
        ...
        request = JSON.parse(msg);
        return rpc.receive(
          `{"jsonrpc": "2.0", "error": {"code": -32601, "message": "Method not found"}, "id": ${request.id}}`,
        );
      };
      try {
    ...
    it("should throw error when timeout", async function () {
      ...
      try {
        ...
        assert.strictEqual(
          e.message,
          `RPC response timeouted. (${JSON.stringify(request)})`,
        );
      }
    });
  ...
  describe("#receive()", () => {
    ...
    it("should batch call methodHandler", async function () {
      ...
      await rpc.receive(
        '[{"jsonrpc": "2.0", "method": "sum", "params": [1,2,4], "id": "1"},{"jsonrpc": "2.0", "method": "notify_hello", "params": [7]}]',
      );
  ...
  describe("#createResponse()", () => {
    it("should return null result", async function () {
      assert.deepStrictEqual(rpc.createResponse(20, null), {
        jsonrpc: VERSION,
        result: null,
        id: 20,
      });
      assert.deepStrictEqual(rpc.createResponse("test", undefined), {
        jsonrpc: VERSION,
        result: null,
        id: "test",
      });
